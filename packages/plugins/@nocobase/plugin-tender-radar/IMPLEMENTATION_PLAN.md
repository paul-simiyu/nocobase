...
**Host.** The platform is served at `https://proj.simpauldesign.com`, which is
...
The host needs no special work: `proj.simpauldesign.com` resolves, to the same address
as `crm.simpauldesign.com`, and a wildcard certificate for `*.simpauldesign.com` covers
it. Should a `www.` form ever be wanted, note that `www.proj.simpauldesign.com` is four
labels and that wildcard would not cover it — serve it as an edge redirect rather than
as a second origin.
